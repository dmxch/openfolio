...
- **CH-Ertragsverzeichnis (MVP)** — ⛔ Kill-Gate gemessen = NO-GO (Steuerjahr 2025: 0 gebuchte Erträge). **Re-Gate: ein vollständig gebuchtes Steuerjahr (2026 → ~Anfang 2027).** (`DESIGN_ch_tax.md`)
- **ICTax-Quelle für thesaurierende Fonds (EIMI, IB01)** — Daten-technisch machbar (Bulk-XML, OSS-Referenz `vroonhof/opensteuerauszug`), Deep-Research GO. **Echtes Vor-Build-Gate = SIX-Financial-Lizenz** (darf ein multi-user self-hosted Tool die Steuerwerte ziehen/zeigen? ungeklärt). (`DESIGN_ch_tax.md` + ICTax-Spike)
- **Wertschriftenverzeichnis / Jahresend-Steuerwert** — braucht PositionSnapshot oder On-Demand-Replay; Daten-Reife ~40 %. Pivot-Option, datenunabhängig von Dividenden. (`DESIGN_ch_tax.md`)
- **DA-1-Prefill + Verrechnungssteuer-Rückforderungs-Status** — Per-Country-Aggregation + Treaty-Lookup; Folge-Welle nach Ertragsverzeichnis. (`DESIGN_ch_tax.md`)
- **Gewerbsmässiger-Wertschriftenhändler-Frühwarnung (ESTV KS 36)** — schützt die CH-Kapitalgewinn-Freiheit; eigenständiger Scope. (`DESIGN_ch_tax.md`, §4.2)
- **Säule-3a-/Vorsorge-Cockpit** — 0 % Daten heute, eigenes Feature (~20–30 h). (`DESIGN_ch_tax.md`, §4.2/4.3)
...
- **Branchen-Flow-Seite (rvol-Fluss-Signal)** — Bau deferred bis rvol-Coverage voll (~Anfang Juli 2026), dann Re-Gate. turnover_ratio ist Level ohne Forward-Kante. ([[project_branchen_flow_killgate]])
- **Freshness Phase 1 (Signal-Alter-Filter)** — Kill-Gate-Datum 17.6. verstrichen; **Entscheid ausstehend**: wurden ≥2 Bewertungs-Einfluss-Fälle dokumentiert? Wenn nein → Phase 0 zurückbauen. Freshness NIE auto-score-decay. (`freshness_phase1_use_log.md`)
- **Schwur-Filter-Toggle (Iteration 2.6)** — Schwur 1 (SMA150) zu strikt → keine NEW-Hits; Toggle als reiner UI-Filter (kein Score-Change). (`retro_iteration1_2026-05-31.md`)
- **EPS-Scanner: Schwellen-Backtest** — Super-Quartal-Schwellen (25 %/+5pp) live mit Disclaimer, aber nicht forward-return-validiert. Plus offene Design-Fragen: Kriterium-C bei <2 Vorwerten, Restatement-Handling, YoY-Cap, Fiscal-Year-Versatz. (`DESIGN_eps_scanner.md` OF-2/3/6/7)
- **FMP→Finnhub estimate_revision-Migration** — 6 US-Ticker FMP-Free 402 (n=6 statt n=33); Finnhub-Probe deckt 6/6. Bau gated/aufgeschoben. (`diagnose_universe_audit_2026-05-21.md`, [[project_fmp_finnhub_estimate_gap]])
...
- **OEF-Country-Default** — Länder-Look-Through-Fallback für Open-End-Funds ohne Holdings-CSV. (Memory)

**D. Performance-Seite & Risiko-Metriken**
- **Risiko-Kennzahlen** (Sharpe/Sortino/Calmar/Volatilität/Information-Ratio + Rolling Returns) — neuer `risk_metrics_service` + `RISK_FREE_RATE_PCT`. Read-only OK; nur falls je als Score-Gewicht → backtest-gated. (`DESIGN_performance_page.md` Phase 3)
- **Per-Bucket Total-Return + Fee-Summary + Alpha/Beta** — `factor_decomposition(bucket_id=...)` durchreichen; neue Endpoints intern+extern, audit-gated. (`DESIGN_performance_page.md` Phase 2)

**E. Trade-Journal — Post-MVP-Ausbau** *(MVP gebaut + deployt; diese Teile bewusst draussen)*
- **Gewichtetes Adhärenz-Scoring + Display** — braucht Forward-Return-Validierung. **Partial-Fill-/Near-Miss-Verlinkung** + **Disziplin-Heatmap/Adhärenz-Trend** — brauchen 6m+ Trade-Kadenz. **`from-rebalancing`-Button** (Plan-Capture aus dem Cockpit). (`DESIGN_trade_journal.md`)
...
- **Per-Quelle Symbol-Resolution-Mapping** (Portfolio-Ticker → yfinance/finnhub/TV) — strukturell wichtig; behebt ROG.SW→ROP.SW (kosmetisch, shares=0) + künftige Non-US-Fälle. (`SPIKE_SIX_COVERAGE.md`)
...
- **Golden-Master-Test-Pin** für `real_estate` mit `shares>0` (Ausschluss-Guard existiert im `snapshot_service`, ist aber noch nicht im Golden-Master gepinnt). (Memory [[project_golden_master_invariants]])
- **Code-Hygiene (Sammel):** Locale-Bypass (`toLocaleString` statt `format.js`, ~15 Dateien), vereinzelte Umlaut-Schäden — niedrige Prio, läuft teils im Redesign mit. (`REVIEW-codebase-2026-06-10.md`)

**G. Frontend/Design-Vollständigkeit** — fehlende Screens/Modals/Zustände (StockDetail, Settings-Tabs, Command-Palette, Skeleton-/Leer-/Fehlerzustände, Watchlist-18-Punkte-Checkliste, volle Filter-Sidebars u. v. m.). Rohdesign existiert; **läuft in separater Session.** (`DESIGN_PROMPT_fehlende_screens.md`, `DESIGN_PROMPT_verfeinerung.md`)

**G2. Mobile / Responsive Design (NEU, 28.06.2026 — kehrt die bisherige Desktop-only-Haltung um)** — bisher ist OpenFolio bewusst **Desktop-only** (CLAUDE.md-Konvention, alle Design-Prompts „Desktop-only", Memory [[project_desktop_only]]); das laufende Redesign ist desktop-first ohne Breakpoints gebaut. Neues strategisches Ziel: die App auch auf dem Handy nutzbar machen. **Offene Scope-Entscheidungen (vor Bau zu klären):** (a) **Form** — responsive Web (Tailwind-Breakpoints aufs bestehende Designsystem) vs. PWA (installierbar, offline-Cache) vs. native App; (b) **Umfang** — zuerst eine **read-first Mobile-Ansicht** (Portfolio/Performance/Watchlist/Alerts ansehen) statt voller Schreib-Parität, oder gleich alles; (c) **Designsystem** — Breakpoints + mobile Navigation (Sidebar→Bottom-Nav/Drawer), Tabellen→Karten-Stacks, Touch-Targets als eigene Schicht ergänzen. **Empfehlung:** responsive Web + read-first, als eigene Designsystem-Erweiterung nach Abschluss des Desktop-Redesigns (sonst doppelte Nacharbeit). Grosser, querschnittlicher Aufwand — eigener Scope/Designrunde. (Maintainer-Wunsch 28.6.)
...
**I. Historische Audit-Reports (abgelöst, nur bei Bedarf gegenprüfen)** — `REVIEW-codebase-2026-06-10.md` (C1/H1–H11/M1–M17) und `AUDIT-v0.39.1-2026-05-17.md` sind **vor v0.49.0** entstanden; die kritischen Befunde (Import-Confirm-Ownership, GBX/GBp, PE-Metadata, Breakout-Zustellung …) sind seither gefixt und durch das stehende `@openfolio-audit`-Gate + den v0.49.0-Audit (1453 Tests grün) abgelöst. Bei Bedarf einzelne Rest-Nits dort nachschlagen — NICHT pauschal als offen behandeln.
