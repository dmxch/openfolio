...
- **FIRE-/Kapital-Projektion (Vorausschau, NEU — interaktiv):** real (inflationsbereinigt) Projektion `Kapital×(1+r)+Sparrate` → FIRE-Zahl (Ausgaben/SWR), Jahre-bis-FIRE, Deckung + Kurve. Annahmen vom User gesetzt (Basis=Netto-Vermögen, SWR 4 %, 5 % real, Sparrate 40k), im Card live-änderbar (localStorage). Kapitalbasis-Hierarchie liquid ⊂ +Vorsorge ⊂ Netto-Vermögen; Disclaimer „inkl. Eigenheim/PE überzeichnet (illiquide)". `/api/analysis/fire-projection` (+ External-Parität) + `FireProjectionCard`. Adversarial geprüft (PE-Nuance: Disclaimer+Test ergänzt). *(committet, Deploy ausstehend)*
