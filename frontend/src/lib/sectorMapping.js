 ...
 * Must stay in sync with backend/services/sector_mapping.py
 */
...
export const FINVIZ_SECTORS = SECTOR_ORDER.slice(0, 11)

// All sectors including custom
export const SECTORS = SECTOR_ORDER

// Multi-Sector industries (trigger ETF sector weight UI)
export const MULTI_SECTOR_INDUSTRIES = ['Broad Market ETF', 'Sector ETF', 'Thematic ETF']

// Grouped: sector → sorted industries[]
export const SECTORS_WITH_INDUSTRIES = {}
for (const [industry, sector] of Object.entries(INDUSTRY_TO_SECTOR)) {
  if (!SECTORS_WITH_INDUSTRIES[sector]) SECTORS_WITH_INDUSTRIES[sector] = []
  SECTORS_WITH_INDUSTRIES[sector].push(industry)
}
for (const sector of Object.keys(SECTORS_WITH_INDUSTRIES)) {
  SECTORS_WITH_INDUSTRIES[sector].sort()
}
