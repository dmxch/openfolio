 ...
 * - pos_to_pos: Prozentwert (gruen bei Wachstum, danger bei Schrumpfung)
...
const FLAG_PILLS = {
  ...
  zero_basis: { label: 'Basis 0', cls: 'bg-surface border border-border-2 text-text-muted', title: 'Vorjahresquartal = 0, kein YoY berechenbar' },
}
...
export default function EpsYoyBadge({ yoyGrowthPct, yoyFlag }) {
  if (yoyFlag === 'pos_to_pos' && yoyGrowthPct !== null && yoyGrowthPct !== undefined) {
    ...
    return (
      <span
        className={`font-mono text-[12px] tabular-nums font-medium ${positive ? 'text-success' : 'text-danger'}`}
  ...
  return (
    <span className={`inline-flex items-center rounded-[5px] px-[7px] py-[3px] text-[10.5px] font-medium leading-none ${pill.cls}`} title={pill.title}>
      {pill.label}
    </span>
  )
}
