...
async def analyze_csv(
    ...
    try:
        return await analyze_csv_structure(content, file.filename or "", db, user.id)
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e))
    except Exception as e:
        logger.error(f"CSV analysis failed for {file.filename}: {e}", exc_info=True)
        raise HTTPException(status_code=502, detail="Analyse fehlgeschlagen")
