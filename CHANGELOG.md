...
- **Komplettes UI-Redesign.** Das gesamte Frontend wurde auf eine neue, ruhigere
  Designsprache umgestellt: dunkles Cockpit-Theme mit abgestimmten Farb-Tokens,
  IBM Plex Sans/Mono (tabellarische Ziffern), einheitliche Karten, sticky
  Seiten-Header und ein wiederverwendbares Komponenten-System (PageHeader, Card,
  StatTile, Filter-Chips, Badges, Ticker-Chips, Buttons, Logo). Alle Seiten, Modals
  und Zustände (Laden/Leer/Fehler) wurden überführt — Datenanbindung und
  Berechnungen blieben dabei unverändert.
- **Neues Logo** — geometrische Wortmarke mit offenem Ring (Blau→Teal).
- **Performance verdichtet** — die Auswertungen sind nun in fünf Sub-Tabs gegliedert
  (Übersicht, Rendite, Risiko, Allokation, Cashflow).

### Hinzugefügt

- **Worker-Liveness pro Job** — Heartbeat je Hintergrund-Job plus stündlicher
  Silent-Stale-Detektor, einsehbar im Admin-Bereich.
- **Per-Signal-Backtest als Worker-Job** — monatlicher Lauf akkumuliert die
  Forward-Return-Historie je Signal über mehrere Marktphasen.
- **FIRE-Annahmen serverseitig** — werden pro Konto gespeichert und gelten
  geräteübergreifend statt nur lokal.
- **Dividenden-Vorausschau auf Abruf** — manueller, gedrosselter Neuberechnungs-Button
  in der Card (intern + externe API).

### Behoben

- **Changelog-Seite war auf Production leer** — die `CHANGELOG.md` lag ausserhalb des
  Frontend-Docker-Build-Kontexts und die generierte Kopie war ignoriert; die
  versionierte `public/changelog.md` wird nun mit ausgeliefert.
- **Production-Deploy-Skript** — Abbruch durch SIGPIPE im Git-Schritt (`git log | head`
  unter `pipefail`) behoben.
- Letzte hartkodierte Farben durch Design-Tokens ersetzt.

### Sonstiges

- Verbindliches **Design-System** dokumentiert (`frontend/DESIGN_SYSTEM.md` + CLAUDE.md);
  der Pre-Release-Audit prüft neue/geänderte UI auf Design-Konformität.
