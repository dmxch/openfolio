...
| GET | `/performance/history?period=1m\|3m\|ytd\|1y\|all&benchmark=^GSPC&raw=false&liquid=false&bucket_id=` | History (tägliches `portfolio_indexed`). `raw=true` → ungedownsamplete Tageskurve (keine 5-Tage-Ausdünnung), verankert an echter Inception (erste Transaktion statt 2000-Default), kein synthetisches Pre-Inception. `liquid=true` → nur Rendite-Risikobuch (Cash + Vorsorge raus; stock/etf/bond/crypto/commodity inkl. Gold+BTC), damit konstanter Ballast Faktor-Betas/Vol nicht dämpft. `bucket_id` (v0.48) skopiert die Kurve auf die Positionen eines Buckets (gleiche `portfolio_indexed`-Methodik). PE + Immobilien immer ausgeschlossen. Die Reihe beginnt **immer an der echten Inception** (erste Transaktion bzw. erster Edelmetall-Kauf), auch bei `period=all` — nie davor. `downsampled` + `sample_interval_days` weisen aus, ob die Reihe ausgedünnt ist (ab >1 Jahr Fenster **mindestens** 5 Tage Abstand — nominal: der letzte Punkt **und nachgetragene Extrempunkte** können 1–4 Tage neben ihren Nachbarn liegen, auch mitten in der Reihe): **randgenaue Rechnungen (Quartals-/Monatsgrenzen) gehören auf `raw=true` oder einen kürzeren Zeitraum.** Es drohen zwei Fehlerklassen, und sie sind **spiegelbildlich** — aus „das Datum stimmt ungefähr“ folgt nie „die Zahl stimmt ungefähr“, und umgekehrt genauso wenig:

| | Datumsfehler | Wertfehler |
|---|---|---|
| **Extremum in einem Teilfenster** | **unbeschränkt** — fällt der wahre Spitzentag zwischen zwei Rasterpunkte, gewinnt irgendein anderer Punkt des Fensters. Über 14 Quartale gemessen: 9 weichen ab, schlimmster Fall **72 Tage** (2024-Q3) | klein (0.76 Indexpunkte) |
| **Rand-/Stichtagswert** | eng — die echten Quartalsenden fehlen im Raster, typisch **1 Tag** | **gross** — Q1 2026 rechnet sich zu −6.04 % statt −4.23 %, also **1.80 pp** Fehler, obwohl beide Ränder nur je einen Tag danebenliegen: die Randfehler wirken gleichgerichtet und multiplizieren sich |

Der Wertfehler am Rand skaliert also **nicht** mit der Datumslücke. Ein Tag Versatz genügt, um über AKTIV/INAKTIV einer −6-%-Schwelle zu entscheiden. Das **globale** Hoch und Tief der Reihe (nach `value` und `portfolio_indexed`) bleiben beim Ausdünnen immer erhalten — deren Höhe *und* Datum sind also exakt. **Nicht** garantiert sind lokale Extrema: Drawdown-**Episoden** (Peak → Trog, Zwischenhochs) gehören weiter auf `raw=true` oder auf `/performance/drawdown`, das serverseitig immer auf der vollen Tagesreihe rechnet. Benchmark-Extrema (`benchmark_indexed`) sind von der Garantie ausgenommen. **Ein Peak-Datum ist ohne die Serie, auf der es gemessen wurde, wertlos:** `value` ist der rohe CHF-Wert und wächst mit jeder Einzahlung mit, `portfolio_indexed` ist cashflow-bereinigt. Auf diesem Portfolio liegt das Allzeithoch nach `value` am 22.07.2026, nach `portfolio_indexed` am 13.08.2025 — beides korrekt, beides eine andere Frage. Wer ein Peak-Datum zitiert, nennt die Serie dazu.

**Zusage an Konsumenten:** `raw=true` bleibt erhalten, ebenso `downsampled` und `sample_interval_days` in der Antwort. Wer randgenau rechnen muss, kann sich darauf verlassen. **Benchmark ab v0.62.0 in CHF** (`benchmark`, `benchmark_indexed`, `summary.benchmark_return_pct`) — vorher Notierungswährung neben einer CHF-Portfolio-Reihe; ist die Notierungswährung oder der FX-Kurs nicht beschaffbar, entfällt das Overlay statt währungsgemischt zu erscheinen |
| GET | `/performance/monthly-returns?bucket_id=` | Modified-Dietz Monatsrenditen, Jahres-Total = XIRR/MWR — **die Verkettung der Monate ergibt daher nicht das Jahres-Total**; das ist Korrektheits-Invariante 1, kein Rundungsfehler. `months_method`/`annual_totals_method` weisen die Verfahren aus, `annual_totals_dietz_fallback_years` nennt Jahre ohne berechenbares XIRR (dort wurden die Monate verkettet) — Achtung, Int-Liste, während die Schlüssel von `annual_totals` im JSON Strings sind. Achtung: beim Benchmark-Endpoint trägt dasselbe Feld `annual_totals` **die Verkettung** — die Namensgleichheit lädt zur Verwechslung ein. `bucket_id` (v0.53) skopiert auf einen Bucket (identisch zu `/buckets/{id}/monthly-returns`, TWR aus Bucket-Snapshots); vorher wurde der Param still ignoriert und das Gesamtportfolio geliefert |
...
| GET | `/performance/risk-metrics?period=1y\|2y\|3y\|5y\|all&benchmark=^GSPC&bucket_id=` | **v0.48** — Risiko-Kennzahlen (Sharpe/Sortino/Calmar/Volatilität/Information-Ratio + Rolling-Returns + Max-Drawdown) aus der cash-flow-bereinigten Index-Reihe. `risk_free_rate_pct` aus `RISK_FREE_RATE_PCT`. Default `5y`. `bucket_id` skopiert auf einen Bucket. Bei zu wenig Historie: 422. `rolling_returns.1y` ist `null`, wenn die Historie kürzer als ein Jahr ist — vor v0.64.0 stand dort ein Wert über 252 **Kalender**tage (gut acht Monate), als „1 Jahr" etikettiert. `periods_per_year` weist die verwendete Annualisierungs-Frequenz aus. **Ab v0.64.0 korrekt annualisiert** — die Reihe ist kalendertäglich, wurde vorher aber mit der 252-Handelstags-Konvention annualisiert (ein Jahr galt als 1.45 Jahre); alle annualisierten Grössen inkl. Sharpe/Sortino/Calmar/IR/TE waren dadurch zu niedrig. **Information Ratio, Tracking Error und `benchmark_annualized_return_pct` sind ab v0.62.0 währungskonsistent** — sie leiten sich aus der jetzt CHF-basierten Benchmark-Reihe von `/performance/history` ab. |
