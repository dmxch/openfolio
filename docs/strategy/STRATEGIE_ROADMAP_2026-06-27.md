...
- **Schwur-Filter-Toggle (Iteration 2.6)** — Schwur 1 (SMA150) zu strikt → keine NEW-Hits; Toggle als reiner UI-Filter (kein Score-Change). (`docs/research/retro_iteration1_2026-05-31.md`)
- **EPS-Scanner: Schwellen-Backtest** — Super-Quartal-Schwellen (25 %/+5pp) live mit Disclaimer, aber nicht forward-return-validiert. Plus offene Design-Fragen: Kriterium-C bei <2 Vorwerten, Restatement-Handling, YoY-Cap, Fiscal-Year-Versatz. (`docs/design/DESIGN_eps_scanner.md` OF-2/3/6/7)
...
- **Risiko-Kennzahlen** (Sharpe/Sortino/Calmar/Volatilität/Information-Ratio + Rolling Returns) — neuer `risk_metrics_service` + `RISK_FREE_RATE_PCT`. Read-only OK; nur falls je als Score-Gewicht → backtest-gated. (`docs/design/DESIGN_performance_page.md` Phase 3)
- **Per-Bucket Total-Return + Fee-Summary + Alpha/Beta** — `factor_decomposition(bucket_id=...)` durchreichen; neue Endpoints intern+extern, audit-gated. (`docs/design/DESIGN_performance_page.md` Phase 2)
...
- **Code-Hygiene (Sammel):** Locale-Bypass (`toLocaleString` statt `format.js`, ~15 Dateien), vereinzelte Umlaut-Schäden — niedrige Prio, läuft teils im Redesign mit. (`docs/audits/REVIEW-codebase-2026-06-10.md`)
