...
## Umsetzungs-Stand (laufend aktualisiert — letzte Aktualisierung 01.07.2026)
...
> **🔧 /analysis/levels → Swing-Low-Leiter für Trailing-Stops (01.07.2026, lokal auf Branch `feat/analysis-levels-swing-ladder`, Deploy ausstehend).** Umsetzung der Finance-Workspace-Spec (`2026-07-01_openfolio_analysis_levels_swing_ladder_spec.md`): der Support/Resistance-Endpoint lieferte bisher nur 52W-Serien-Extreme (AMAT „Support" = 155.56 = Mehrjahres-Tief, −79 % — als Trail-Anker wertlos). Jetzt **native Swing-Struktur**: geordnete `swing_lows`/`swing_highs` (nächstes-zu-Spot zuerst) mit Metadaten (date/type/touches/dist_pct/dist_atr), `atr_22`, `highest_high_22` (Chandelier-Cap), `gap_bases`/`reaction_low_k1` (Parabel-Fallback), `as_of`. **Additiv** (Maintainer-Entscheid): `support`/`resistance` bleiben die 52W-Close-Extreme → UI-Label „52W-Tief/Hoch" bleibt wahr, **0 Frontend-Change**, keine stille Definitions-Drift; `*_historical` tragen die Leiter-Preise. Konsument = Adaptive Structure Trail (`swing_lows[0]` = Anker) → nimmt der Stop-Geometrie die per-Position-TradingView-Abhängigkeit (headless-tauglich). Adversarial-Review (5 Lenses) fing **1 MED**: im close-only-Fallback ist high==low==close → Gap-up-Erkennung degenerierte zu „jeder Up-Tag" (Phantom-Anker ~1-2 % unter Spot) → gegatet auf echtes OHLC + Regressionstest. 18 Tests, 101 grün; **AMAT-Anker 609.70 / CAT 970.05 live verifiziert**; intern+extern (Parität automatisch), API-Doku [[feedback_api_parity_and_doc]] gezogen. Commit `b34bb2e`.
>
> **§8 (`/analysis/trail-suggestion` — kompletter AST server-seitig) — Kill-Gate gemessen = NO-GO (01.07.).** Drei Blocker: (1) Openfolio hat **kein natives RSI/Stochastic** (0 Treffer in `services/`) → der Verengungs-Score-Faktor „überkauft" (`RSI≥70 ODER Stoch.K≥80`) ist server-seitig nicht rechenbar; die 7e/2c-Werte kommen aus TradingView-MCP. (2) Der AST-SoT (`/portfolio-review` 7e) hält Chandelier-Lock-in + Low-Vol-/Fallback-Ausnahmen bewusst als **FLAG, nie Auto-Stop** („der User entscheidet") — ein Server-`suggested_stop`-Skalar würde vom kanonischen interaktiven Trail **divergieren** (genau das „doppelte Kopien driften", das der SoT verbietet). (3) Das Spec-§8-Beispiel (`suggested_stop:605`) ist selbst **inkonsistent mit 7e** (die Formel liefert 560 via Min-Move-Gate). §1-7 liefert bereits den headless-Anker; §8 erst nach Deploy/Bewährung **und** einem sauberen RSI/Stoch-Bau (Standard-TA, nicht scoring-gated), damit der Server-Score == der interaktive Score.
...
- **Markt-Klima: Bewertung von Risk-On/Off entkoppelt (29.6., lokal auf main).** Auslöser: „6/6 Kriterien bullish, Makro-Gate bestanden — trotzdem Risk Off / Kritisch?". Ursache war Absicht („macro dominates" in `api/market.py`): die Headline zählte ALLE Makro-Indikatoren inkl. **CAPE & Buffett**; da diese strukturell jahrelang erhöht sind (red_count≥2), stand die Ampel quasi-permanent auf „Risk Off", unabhängig vom Trend. Fix (Maintainer-Entscheid „Risk-Treiber + Bewertungs-Badge"): Headline folgt nur noch **taktischen Risk-Treibern** (Trend + VIX + Credit + Zinskurve + Arbeitslosen-Trend, grün = „Risk On"); **CAPE/Buffett** als separates **Bewertungs-Badge** (Warn-Ton, kippt die Headline nicht). Pure `_summarize_indicators` (testbar), `risk_status`/`valuation_status` + per-Indikator `group`; `overall_status` + „Risk Off"-Alert zählen nur noch Risk-Treiber. Verhaltensänderung an einem verfolgten Signal → CHANGELOG + Unit-/API-Tests (`test_macro_indicators_summary`, `test_market_api`). Makro-Gate (informativ) unberührt. **Lehre:** Bewertung ist Kontext, kein taktisches Timing-Signal — nie als harte Risk-Off-Headline.
