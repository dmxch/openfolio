...
- ⬜ **OFFEN (neu 28.6., aus dem Performance-Mockup-Redesign): Dividenden-Forecast als Pro-Monat-Balkendiagramm.** Die Cashflow-Card soll die erwartete Ausschüttung **pro Monat** als Balken zeigen (Claude-Design-Mockup „Erwartete Ausschüttung pro Monat · CHF"). **Gate = Daten:** der `dividend-forecast`-Endpoint liefert aktuell nur 12M-Total (`forecast_12m_chf`) + `by_holding` (forecast/DPS/Zahlungs-Anzahl) — **KEINE Monats-Verteilung**, also kein echtes Monats-Diagramm ohne erfundene Zahlen. **To-do:** Backend erweitern — jeden Zahler aus seiner realen Ausschüttungs-Historie (welche Monate er effektiv zahlte × DPS) auf 12 Monate verteilen → neues `by_month`-Feld am Endpoint (intern + external_v1 + Test, keine geratenen Monate). Dann die Forecast-Card auf Monats-Balken umstellen. Bis dahin bleibt die Card beim Total/Run-Rate. (`dividend_forecast_service.py` + Dividenden-Historie/Ledger)
